...
def fetch_prices(symbol, out_name):
    ...
    # 需要提前打开代理，端口7890
    df = yf.download(symbol, start="2001-01-01", end="2023-01-01", proxy="http://127.0.0.1:7890")
...
def check_date(symbol, out_name):
    ...
    # 检查第一个日期是否是 2001-01-02 and 最后一个 2022-12-30
    if df['Date'].iloc[0] == pd.Timestamp('2001-01-02') and df['Date'].iloc[-1] == pd.Timestamp('2022-12-30'):
        return True
    else:
        # 删除相应的股票数据文件
        os.remove(out_name)
